...
namespace Signal_Windows.Storage
{
    public sealed class LibsignalDBContext : DbContext
    {
        ...
        public static event EventHandler<IdentityKeyChangeEventArgs> IdentityKeyChange;
            ...
            lock (DBLock)
            {
                using (var ctx = new LibsignalDBContext())
                {
                    ...
                    else if (old.IdentityKey != identity)
                    {
                        ...
                        IdentityKeyChange?.Invoke(null, new IdentityKeyChangeEventArgs(address.Name));
                    }
                    ctx.SaveChanges();
                }
            }
        ...
        private static List<PreKeyRecord> GeneratePreKeys()
        {
            ...
            for (uint i = 1; i < SignalConstants.PREKEY_BATCH_SIZE; i++)
            {
                uint preKeyId = (SignalConstants.Store.PreKeyIdOffset + i) % Medium.MAX_VALUE;
            ...
            UpdatePreKeyIdOffset((SignalConstants.Store.PreKeyIdOffset + SignalConstants.PREKEY_BATCH_SIZE + 1) % Medium.MAX_VALUE);
            return records;
        }
        ...
        private static SignedPreKeyRecord generateSignedPreKey(IdentityKeyPair identityKeyPair)
        {
            try
            {
                ...
                SignedPreKeyRecord record = new SignedPreKeyRecord(SignalConstants.Store.NextSignedPreKeyId, (ulong)DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond, keyPair, signature);

                StoreSignedPreKey(SignalConstants.Store.NextSignedPreKeyId, record);
                UpdateNextSignedPreKeyId((SignalConstants.Store.NextSignedPreKeyId + 1) % Medium.MAX_VALUE);
                return record;
            }
    ...
    public class SignalDBContext : DbContext
    {
        ...
        public static event EventHandler<UpdateMessageStatusEventArgs> MessageStatusUpdated;
        public static event EventHandler<NewSignalGroupEventArgs> NewSignalGroup;
        public static event EventHandler<SignalGroupUpdatedEventArgs> SignalGroupUpdated;
        public static event EventHandler<NewSignalContactEventArgs> NewSignalContact;
        public static event EventHandler<SignalContactUpdatedEventArgs> SignalContactUpdated;
        ...
        public static void UpdateMessageStatus(SignalMessage outgoingSignalMessage)
        {
            SignalMessage m;
            lock (DBLock)
            {
            ...
            MessageStatusUpdated?.Invoke(null, new UpdateMessageStatusEventArgs(m));
        }

        public static void IncreaseReceiptCountLocked(SignalServiceEnvelope envelope)
        {
            SignalMessage m;
            bool set_mark = false;
            lock (DBLock)
        ...
        public static SignalGroup GetOrCreateGroupLocked(string groupId, long timestamp)
        {
            ...
            lock (DBLock)
            {
                ...
                if (is_new)
                {
                    NewSignalGroup?.Invoke(null, new NewSignalGroupEventArgs(dbgroup));
                }
            }
            return dbgroup;
        }
        ...
        public static SignalGroup InsertOrUpdateGroupLocked(string groupId, string displayname, string avatarfile, bool canReceive, long timestamp)
        {
            ...
            lock (DBLock)
            {
                ...
                {
                    SignalGroupUpdated?.Invoke(null, new SignalGroupUpdatedEventArgs(dbgroup));
                }
            }
            return dbgroup;
        }
        ...
        public static SignalContact GetOrCreateContactLocked(string username, long timestamp)
        {
            ...
            lock (DBLock)
            {
                ...
                if (is_new)
                {
                    NewSignalContact?.Invoke(null, new NewSignalContactEventArgs(contact));
                }
            }
            return contact;
        }

        public static void InsertOrUpdateContactLocked(SignalContact contact)
        {
            bool is_new = false;
            lock (DBLock)
            {
                ...
                {
                    SignalContactUpdated?.Invoke(null, new SignalContactUpdatedEventArgs(contact));
                }
            }
        }
